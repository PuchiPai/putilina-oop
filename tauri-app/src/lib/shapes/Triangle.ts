...
export class Triangle extends Shape {
    // Локальные вершины (относительно центра)
    ...
    // Вспомогательная функция знака (cross product)
    ...
    override clone(): Triangle {
        const cloned = new Triangle(
            this.id + '_copy',
            { ...this.transform },
            this.localA.x + this.transform.x,
            this.localA.y + this.transform.y,
            this.localB.x + this.transform.x,
            this.localB.y + this.transform.y,
            this.localC.x + this.transform.x,
            this.localC.y + this.transform.y,
        );
    ...
    override toJSON(): any {
        const absA = { x: this.localA.x + this.transform.x, y: this.localA.y + this.transform.y };
        const absB = { x: this.localB.x + this.transform.x, y: this.localB.y + this.transform.y };
        const absC = { x: this.localC.x + this.transform.x, y: this.localC.y + this.transform.y };
        return {
            id: this.id,
            type: 'triangle',
            transform: { ...this.transform },
            points: [absA, absB, absC],
